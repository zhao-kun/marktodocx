...
markdocx 当前仍以源码构建的方式分发。现在还没有面向最终用户的 Chrome Web Store、VS Code Marketplace、npm 二进制包或 GitHub Releases 制品，因此四种宿主都需要从本仓库本地构建。
...
当前所有安装流程都从本地源码 checkout 开始。在仓库根目录执行：
